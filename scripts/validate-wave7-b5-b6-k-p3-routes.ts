...
if (fs.existsSync(PM)) {
  ...
  eq({ prerender: routes.length, html }, { prerender: BASELINE_BUILD.prerender + expected + W8.build + R2.build, html: BASELINE_BUILD.html + expected + W8.build + R2.build }, `build totals == frozen baseline + ${expected}${W8.build ? ` + later Wave-8 ${W8.build}` : ""} + later R2 categories ${R2.build}`);
