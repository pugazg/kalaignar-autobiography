...
if (fs.existsSync(pm)) {
  ...
  const w7bBuild = (w7bLive ? 225 : 0) + (LIBRARY_WORKS.some((w) => w.id === "kuraloviyam") ? W7K_ALL.build : 0) + W8.build + R2.build; // + later Wave-7 B5/B6/K routes once published + Wave-8 direct routes
