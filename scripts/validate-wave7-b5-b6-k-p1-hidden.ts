...
if (stage === "hidden") {
  ...
  eq(works.length, BASELINE.catalogue + 101, "published: catalogue == 232 + 101");
  eq(shelf("speeches"), BASELINE.speeches + 100, "published: Speeches == 17 + 100");
  eq(shelf("literary-commentary"), BASELINE.literaryCommentary + 1, "published: Literary Commentary == 2 + 1");
  eq({ drama: shelf("drama"), fiction: shelf("fiction"), essays: shelf("essays-articles"), cinema: shelf("cinema-writing") }, { drama: 10, fiction: 162, essays: 15, cinema: 10 }, "published: other shelves unchanged");
