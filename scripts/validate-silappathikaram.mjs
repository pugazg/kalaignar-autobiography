...
eq("play holds 39 units", play.readingUnits.length, 39);
// The generalized Play model makes this edition's structure explicit rather than implied, so the
// regression now also pins the structural fields themselves.
eq("structure kind is a scene sequence", play.structureKind, "scene-sequence");
eq("scan provenance is per-unit", play.scanProvenance, "per-unit");
check("no opening note is invented for this edition", !("openingNote" in play));
check("no reading unit is a continuous body", !play.readingUnits.some((s) => s.kind === "continuous-body"));
const numbered = play.readingUnits.filter((s) => s.kind === "scene");
const tableau = play.readingUnits.filter((s) => s.kind === "closing-tableau");
...
check("no scene anywhere is numbered 39", !play.readingUnits.some((s) => s.order === 39));
...
eq("slugs are the source filename stems", play.readingUnits.map((s) => s.slug), sceneFiles.map((f) => f.replace(/\.md$/, "")));
...
// ── 4. SPEAKER LABELS ARE PRINTED AUTHORITY ──────────────────────────────────────────────────────
{
  ...
  const used = new Set(play.readingUnits.flatMap((s) => s.tamil.units.filter((u) => u.kind === "dialogue" && u.speakerAsPrinted).map((u) => nfc(u.speakerAsPrinted))));
  ...
  const seps = new Set(play.readingUnits.flatMap((s2) => s2.tamil.units.filter((u) => u.kind === "dialogue" && u.speakerAsPrinted !== null).map((u) => u.speakerSeparator)));
  check("the printed label separator is carried verbatim, not regularised", seps.size > 1, `separators seen: ${JSON.stringify([...seps])}`);
  check("unlabelled speeches carry no separator", play.readingUnits.every((s2) => s2.tamil.units.every((u) => u.kind !== "dialogue" || u.speakerAsPrinted !== null || u.speakerSeparator === null)));
}

// ── 5. UNLABELLED SPEECH IS RECORDED AS ABSENCE, NOT ATTRIBUTED ──────────────────────────────────
{
  const unl = play.readingUnits.flatMap((s) => s.tamil.units.filter((u) => u.kind === "dialogue" && u.speakerAsPrinted === null));
...
{
  const sds = play.readingUnits.flatMap((s) => [...s.tamil.units, ...s.english.units].filter((u) => u.kind === "stage-direction"));
  ...
  check("no unlabelled dialogue unit is delimiter-shaped",
    play.readingUnits.flatMap((s) => s.tamil.units).filter((u) => u.kind === "dialogue" && u.speakerAsPrinted === null)
  ...
  eq("provenance counts Tamil stage directions", prov.archiveDerived.tamilStageDirections,
    play.readingUnits.reduce((n, s) => n + s.tamil.units.filter((u) => u.kind === "stage-direction").length, 0));
}

// ── 7. THE SCAN-88 OBSTRUCTION ───────────────────────────────────────────────────────────────────
{
  const all = play.readingUnits.flatMap((s) => [...s.tamil.units, ...s.english.units]).map((u) => u.text).join("\n");
  ...
  check("it appears in the Tamil layer", play.readingUnits.some((s) => s.tamil.units.some((u) => u.text.includes(OBSTRUCTION))));
  check("it appears in the English layer", play.readingUnits.some((s) => s.english.units.some((u) => u.text.includes(OBSTRUCTION))));
...
{
  const body = play.readingUnits.flatMap((s) => [...s.tamil.units, ...s.english.units]).map((u) => u.text).join("\n");
  ...
  const notes = play.readingUnits.flatMap((s) => s.english.notes);
  ...
  check("every scene has both note kinds", play.readingUnits.every((s) => s.english.notes.some((n) => n.kind === "translation-note") && s.english.notes.some((n) => n.kind === "interpretive-note")));
  check("interpretive notes are typed apart from translation", notes.every((n) => n.kind === "translation-note" || n.kind === "interpretive-note"));
  check("no note leaked into a unit", play.readingUnits.every((s) => s.english.units.every((u) => !u.text.includes("interpretive context"))));
}
...
// ── 9. THE 2009 PUBLISHED WITNESS IS NEVER READER CONTENT ────────────────────────────────────────
{
  ...
  const readerContent = nfc(JSON.stringify(play.readingUnits.map((s2) => ({ u: [...s2.tamil.units, ...s2.english.units].map((u) => u.text), n: s2.english.notes }))));
  ...
  const bodyOnly = play.readingUnits.flatMap((s) => s.english.units).map((u) => u.text).join("\n");
  check("no witness wording in the English reading body", !/Tale of the Anklet|Macmillan|Bharathiar/.test(bodyOnly));
  check("every scene declares the witness was not used", play.readingUnits.length === 39);
  for (const f of sceneFiles) {
...
// ── 10. PRINTED FOLIOS AND SETTINGS ARE NEVER INVENTED ───────────────────────────────────────────
{
  ...
  for (const s of play.readingUnits) for (const u of [...s.tamil.units, ...s.english.units]) for (const p of u.sourcePages) {
  ...
  check("some folios are null and stay null", play.readingUnits.some((s) => s.tamil.units.some((u) => u.sourcePages.some((p) => p.printedPage === null))));
  ...
  for (const s of play.readingUnits) {
  ...
  eq("scenes printing no setting", prov.archiveDerived.scenesWithoutPrintedSetting, play.readingUnits.filter((s) => s.settingTa === null).length);
  check("at least one scene prints no setting", play.readingUnits.some((s) => s.settingTa === null));
  check("every unit carries source provenance", play.readingUnits.every((s) => [...s.tamil.units, ...s.english.units].every((u) => u.sourcePages.length > 0)));
  check("cited scans lie in the body range", play.readingUnits.every((s) => [...s.tamil.units, ...s.english.units].every((u) => u.sourcePages.every((p) => p.scan >= play.bodyScans.from && p.scan <= play.bodyScans.to))));
}
