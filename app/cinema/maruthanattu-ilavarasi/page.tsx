...
export function generateMetadata(): Metadata {
  ...
  const description = `${w.titleEn} (${w.titleTa}) — the printed screenplay/dialogue booklet in the original Tamil with a project-created English reading layer. ${w.scenes.length} archival scene segments.`;
  return { title, description, openGraph: { title, description }, twitter: { title: w.titleEn, description } };
}

export default function MaruthanattuLandingPage() {
  const w = loadWave7Cinema(SLUG);
  if (!w) notFound();
