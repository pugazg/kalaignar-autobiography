...
export function generateMetadata(): Metadata {
  const r = loadReader();
  if (!r) return { title: "Ammayappan — அம்மையப்பன் | Kalaignar Digital Library" };
  const title = `${r.work.titleTa} — ${r.work.titleEn} | Kalaignar Digital Library`;
  const description =
    `${r.work.titleEn} (${r.work.titleTa}) — the printed screenplay/dialogue booklet in the original ` +
    `Tamil with a project-created English reading layer. The ${r.counts.scenes} screenplay segments are ` +
    `archival navigation; the booklet prints no scene numbers.`;
  return { title, description, openGraph: { title, description }, twitter: { title: r.work.titleEn, description } };
}

export default function AmmaiyappanPage() {
  const r = loadReader();
  if (!r) notFound();
  return <AmmaiyappanLanding reader={r} />;
}
